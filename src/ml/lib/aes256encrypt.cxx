...
extern "C" {
value cpp_aes_datasz(value unit)
{
    CAMLparam1(unit);
    return Val_long(Aes::datasz);
}
}

/*
 *   cpp_mk_aes256encrypt : key256 -> key128 -> ptr
 */
extern "C" {
value cpp_mk_aes256encrypt(value vk, value viv)
{
    CAMLparam2(vk, viv);
    CAMLlocal1(res);
    Key256 k(true); k.fromHex(std::string(String_val(vk)));
    Key128 iv(true); iv.fromHex(std::string(String_val(viv)));
    auto p = new AesEncrypt(k, iv);
...
/*
 *   cpp_process_aes256encrypt  : ptr -> int -> b -> int
 */
extern "C" {
value cpp_process_aes256encrypt(value vaes, value vlen, value varr)
{
...
/*
 *   cpp_finish_aes256encrypt  : ptr -> int -> b -> int
 */
extern "C" {
value cpp_finish_aes256encrypt(value vaes, value vlen, value varr)
{
    CAMLparam3(vaes, vlen, varr);
    CAMLlocal1(res);
    long len = Long_val(vlen);
